...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--inv_reg', type=float, help='Inverse of regularization strength; must be a positive float')
    ...
    for comb in itertools.combinations(args.fold_files, len(args.fold_files) - 1):        
        ...
        if not args.use_model:
            clf = linear_model.LogisticRegression(C=args.inv_reg, class_weight={'BAJA+2': 5})
        else :
